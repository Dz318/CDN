...
layui.use(['laydate','dropdown'], function(){
	  var dropdown = layui.dropdown,laydate = layui.laydate;
	  //执行一个laydate实例
	  laydate.render({
		...
		laydate.render({
		 ...
		 //初演示
		  dropdown.render({
		    ...
		    ,click: function(obj){
			  $("#title-center2").text("中建一局第一建筑有限公司-"+obj.title)
	...
	var myChart = echarts.init(total_container,chatTheme,chat_size);
	var app = {};
	...
	myChart.on('click', function(params) {
		layer.open({
			...
			title:params.name,
			area: ['700px', '500px'],
			content:['list.html','no'],
			success:function(layero,index){
				var body = layer.getChildFrame('body', index);
				// 表头
				var cols = [[ 
						  {field: 'id', title: '所在单位', width:80, sort: true, fixed: 'left'}
						  ,{field: 'dept', title: '所在部门', width:80}
						  ,{field: 'name', title: '姓名', width:80, sort: true}
						  ,{field: 'gw', title: '岗位', width:80} 
						  ,{field: 'sign', title: '人员属性', width: 177}
						]];
				$(layero).find("iframe")[0].contentWindow.helo(cols);///得到iframe页的窗口对象，执行iframe页的方法：
		　　　　 alert(iframeWin.count);//调用子页面的方法，得到子页面返回的ids
			}
		})
	...
	$('.back').click(function() {
	    ...
	    parentInfo.pop()
	    initData(parentInfo[parentInfo.length - 1].code)
	})
	var geoJson = [];
	var parentInfo = [{
	    cityName: '全国',
	...
	initData(parentInfo[0].code);
	
	function initData(adcode) {
	...
	function initEchartMap(mapData, sum, pointData){
		...
		var myChart = echarts.init(dotMap);
				...
				title: [
					{
						left: 'center',
						top: 10,
						text: parentInfo[parentInfo.length - 1].cityName + '人员区域分布',
						textStyle: {
							color: 'rgb(179, 239, 255)',
							fontSize: 20,
						},
				...
				geo: {
					...
					tooltip: {
						...
						formatter: (p) => {
							...
							let txtCon =
								"<div style='text-align:left;font-size:14px'>" + p.name + ':<br />人员数量：' + val.toFixed(0) + '</div>';
							return txtCon;
						},
					},
					label: {
				...
				series: [
					...
					},
					{
						...
						symbolSize: function(val) {
							var a = (maxSize4Pin - minSize4Pin) / (480 - 9);
	...
	function echartsMapClick(params) {
	    ...
	    if (!params.data) {
	        ...
	        initData(data.cityCode);
	    }
	}
	// 地图展示-end
	// 各单位人员汇总-start
 	var unit_container_dom = document.getElementById("unit_container");
	var myChart = echarts.init(unit_container_dom,chatTheme,chat_size);
	var app = {};
	...
	unit_container_option = {
	    ...
	    xAxis: {
	        type: 'category',
	        name:'分公司',
	        data: ['公司总部','基础设施', '西南分', '华东分', '山东分', 
			'江苏分', '河南分','安徽分','南方分','安装分','菲律宾分公司'],
			axisLabel:{
				interval:0,
				rotate:40,
				color:'white'
	...
	var myChart = echarts.init(dom,chatTheme,chat_size);
	...
	var myChart = echarts.init(chartDom,chatTheme,chat_size);
	var edu_option;
	var firstEdu = [
	    ...
	    series: [
	        {
	            name: '实际人数',
	            type: 'line',
	            data: [3420, 3514, 3869, 4009, 4362, 4401,4517,4615,4720,4913,5087,5179]
	        },
	        {
	            name: '计划人员',
	            type: 'line',
	            data: [3308, 3420, 4104, 4214, 4471, 4356,4698,4780,4810,5004,5117,5206]
	        }
	    ]
	};
	...
	// 基本工资
	var struture_basicPay = [581,1054,1139,749,1154,1191,932,990,560,1033];
	// 绩效奖金
	var struture_meritPay = [495,322,463,329,374,428,524,600,470,553];
	// 单项事项奖罚
	var struture_itemRP =  [212,256,370,228,264,222,285,296,139,363];
	// 津补贴
	var struture_subsidies = [315,313,355,310,473,428,303,483,150,367];
	// 效益奖金
	var struture_Performance_bonus = [1692,1137,1330,1560,1629,1260,1755,1662,763,561];
	
	// 合计值
	var total_pays = function(){
		var pays = [];
		for(var i = 0;i<struture_basicPay.length;i++){
			pays.push(struture_basicPay[i] + struture_meritPay[i] + struture_itemRP[i] + struture_subsidies[i] + struture_Performance_bonus[i]);
		}
		return pays;
	}
	...
	salary_structure_proportion_option = {
	    ...
	    series: [
	        ...
	        },
	        {
	            name: '效益奖金',
	            type: 'bar',
	            data: struture_Performance_bonus,
				stack: 'total',
	            label: {
	                show: true
	            },
	    ...
	    series: [
	        {
	            name: '25分位值',
	            type: 'line',
	            data: [3960,2603,3178,3176,3555,2888,3624,8193,8496,12603],
	            markPoint: {
	                data: [
	                    {type: 'max', name: '最大值'},
	                    {type: 'min', name: '最小值'}
	        ...
	        },
	        {
	            name: '50分位值',
	            type: 'line',
	            data: [6647,6561,6262,6871,5580,6234,4172,6786,7178,15690],
	            markPoint: {
	                data: [
	                    {type: 'max', name: '最大值'},
	                    {type: 'min', name: '最小值'}
	        ...
	        },
	        {
	            name: '75分位值',
	            type: 'line',
	            data: [7796,7866,8286,8732,6244,7499,9440,3593,6780,18603],
	            markPoint: {
	                data: [
	                    {type: 'max', name: '最大值'},
	                    {type: 'min', name: '最小值'}
	...
	var CI = [782,933,978,882];
	var safe = [497,426,554,454];
	var quality = [490,457,458,263];
	var business = [552,429,564,351];
	var process = [2143,1670,1150,508];
	var ultimately = [1816,1598,1216,720];
	var other = [714,574,318,132];
	var penalty = [206,152,107,19];
	
		
	var total_RP = function(){
		var sum = []
		for(var i = 0;i<CI.length;i++){
			sum.push(CI[i]+safe[i]+quality[i]+business[i]+process[i]+ultimately[i]+other[i]+penalty[i]);
		}
		return sum;
	}
